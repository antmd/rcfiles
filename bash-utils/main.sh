...
read -r -d '' __extra_usage <<-'EOF' || true # exits non-zero when EOF encountered
...
function __b3bp_function_exists() {
    declare -F -- "$@" >/dev/null;
}

function __b3bp_cleanup_before_exit () {
  __b3bp_function_exists __cleanup && __cleanup
  info "Done. Exiting $__base"
}
